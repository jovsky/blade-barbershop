...
const TimeSlot: FC<TimeSlotProps> = ({ dateTime, numberSlots, onDateChange, slotTime, timeHovered, setHoveredSlot }) => {
  ...
  const isSelected = selectedPeriod.length === numberSlots && selectedPeriod.includes(slotTime)

  const notSelectable = !hasAvailableTime && hoveredInterval.includes(slotTime)

  const blockedPeriod = hoveredInterval.includes(slotTime) && hoveredInterval.some((h) => busyTimes.includes(h))
  ...
  return (
    <div
      key={slotTime}
      className={cn('flex justify-center items-center cursor-pointer h-8 border border-zinc-800 rounded select-none', {
...
export default function TimeInput(props: HoursInputProps) {
  ...
  const mapComponent = (slotTime: string, i: number) => (
    <TimeSlot key={i} {...props} slotTime={slotTime} timeHovered={timeHovered} setHoveredSlot={setHoveredSlot} />
  )
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      <div className="flex flex-col gap-3 select-none">
        ...
        <div className="grid grid-cols-8 gap-1">{morning.map(mapComponent)}</div>

        <span className="text-xs uppercase text-zinc-400">Afternoon</span>
        <div className="grid grid-cols-8 gap-1">{afternoon.map(mapComponent)}</div>
